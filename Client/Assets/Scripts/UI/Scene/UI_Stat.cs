...
public class UI_Stat : UI_Base
{
    ...
    public void RefreshUI()
    {
        ...
        Get<Text>((int) Texts.txtName).text = player.name;
        int totalDamage = player.Stat.Attack + player.WeaponDamage;
        Get<Text>((int) Texts.txtAttackValue).text = $"{totalDamage} (+{player.WeaponDamage})";
        Get<Text>((int) Texts.txtDefenceValue).text = $"{player.ArmorDefence}";
    }
